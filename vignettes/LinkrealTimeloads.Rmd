...
```{r initialise folder,results='hide'}

# Set user_data_folder, this can be set to any location
...
```{r import process and impute data, results='hide'}
...
```{r plot and report load}

plot(Output$time,Output$Discharge_m3_per_sec)
plot(Output$time,Output$TSS_mg_per_liter$median_confidence,ylim = c(0,750))

Output$Total_load_kt$median_confidence
...
MRD_ADCP_Setting_Summary_Table <- readRDS(paste0(system.file("extdata", package = "LinkrealTimeloads"),'/ADCP_Setting_Summary_Table.rds'))

# Load ADCP_Setting_Summary_Table for Johnstone River
JRI_ADCP_Setting_Summary_Table <- readRDS(paste0(user_data_folder,'/JRI/ADCP_data/ADCP_Setting_Summary_Table.rds'))
